  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
     const ngosRef = collection(db, "EduCare-ngos");

      const q = query(ngosRef, where("domain", "==", "Education"), where("verified", "==", true));

      const snapshot = await getDocs(q);
      const ngos = snapshot.docs.map(doc => doc.data());

      if (ngos.length === 0) {
        container.innerHTML = '<p>No  Educare Organisations  found.</p>';
        return;
      }
