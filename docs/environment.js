  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
     const ngosRef = collection(db, "Environment");

      const q = query(ngosRef, where("domain", "==", "environment"), where("verified", "==", true));

      const snapshot = await getDocs(q);
      const ngos = snapshot.docs.map(doc => doc.data());

      if (ngos.length === 0) {
        container.innerHTML = '<p>No Environment Care   Organisations  found.</p>';
        return;
      }
